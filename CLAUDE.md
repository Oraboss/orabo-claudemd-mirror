...
├── functions/_middleware.js # Cloudflare Pages Function — edge geo detection; sets orabo_region=regulated|unregulated cookie (Max-Age 43200) + x-orabo-region header. Additive only (await next()), never rewrites HTML/injects scripts, defaults unregulated when request.cf absent
