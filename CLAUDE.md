...
- **Digest auto-send is INTENTIONALLY PAUSED** pending the watched first production digest run. BOTH scheduled triggers are commented out — the GitHub Action `schedule:` block (`workflow_dispatch` left intact) and the `scheduler.js` digest `cron.schedule('0 8 * * 6', …)` registration. This is deliberate, NOT a regression: do not "fix" the disabled cron. The route and its secret-gating, plus the `?testEmail=` and `?dryRun=` manual paths, remain fully live. Re-enable both triggers (uncomment) only after the watched run is signed off.
