...
- **Motion infrastructure (Phase 1 + 2 + 2.1 hotfix + 3):** `css/motion.css` + `js/motion.js` (ES module) — shared utilities for homepage count-up animations and reveal transitions across all four motion phases. Phase 1: 13 `.is-counter` spans; card hover-lift on `.us-tool-card`, `.tool-landing-card`, `.consult-card`, `.blog-teaser-card`. Phase 2: spine reveal (`.spine-section` + `.spine-stagger` on PLAN/VERIFY/LIVE band-headers, vertical `.spine-line` accent between sections via CSS scroll-driven animation), readiness score gauge (`.readiness-gauge` SVG arc animates to 78% + `.is-counter` centre count-up), hero "Verified" gold underline (`.hero-verified` + `.hero-verified-underline` SVG path draws on load), IO threshold lowered to 0.1 (fixes sibling-counter sequential-fire glitch). Counter total: 14. `prefers-reduced-motion` respected throughout. Phase 3–4 consume `initReveal()` and the `.reduced-motion` html class flag. Phase 2.1 hotfix: `css/mobile.css` `[class*="hero"]` overflow-x guard uses `overflow-x: hidden !important` which matched `.hero-verified` inline span and `.hero-verified-underline` SVG at ≤768px, producing a scrollbar artefact and hiding the gold underline; override in `css/motion.css` mobile media query uses `overflow: visible !important` on `.hero-content .hero-verified, .hero-content .hero-verified-underline` — `!important` is required because mobile.css itself uses `!important`. Phase 3: the hero static phone mockup (`.phone-frame`/`.phone-screen`/`.app-*`) was replaced by an autoplay/muted/loop/playsinline `<video class="hero-demo">` (`assets/orabo-demo-loop.webm` VP9 + `assets/orabo-demo-loop.mp4` H.264 sources, `assets/orabo-demo-poster.webp` poster) inside the retained `.hero-mockup` flex wrapper (still hidden ≤1024px). Reduced-motion swaps `<video>`→poster `<img class="hero-demo-poster">` via the `html.reduced-motion` class set in `js/motion.js` PLUS a parse-time `@media (prefers-reduced-motion: reduce)` CSS fallback — the `@media` rule closes the deferred-module autoplay race so `js/motion.js` needs no change. CLS prevented via `aspect-ratio: 390 / 844` on the video element; static phone bezel via CSS `border` + `box-shadow` (no SVG, no animated dimensions). The dead `.phone-frame`/`.app-*`/`.hero-phone-mockup`/`@keyframes phoneFloat` rules were removed from `css/hero.css`. Phase 3 hero demo loop verified on production via authoritative Chrome MCP smoke (device 086858af-5efb-4b66-85cf-3b05cb83d41a, 2026-06-26): WebM-selected autoplay/muted/loop/playsInline on desktop, mobile ≤1024px hide confirmed, reduced-motion video→poster swap live, assets serve 200 (webm 51KB / mp4 58KB / webp 6KB), zero console/CSP errors, total page CLS 0.0146 (video box stable at 260×563, no intrinsic-size jump — the 0.0143 shift is the pre-existing #heroTestimonial recenter under .hero align-items:center, not a video regression). Lighthouse mobile Performance number not run via MCP (no runner); all inputs to it green at ~57KB cold hero payload. Phase closed. **Mandatory mobile verification for all future motion phases:** test at iPhone 14 Pro (390×844), Samsung Galaxy S20 Ultra (412×915), and iPhone SE (375×667) before every commit.
...
| `japa-guide.html` | SEO landing + Japa Readiness Guide lead magnet — Email + Destination form (`.japa-guide-form`); success state = tappable `.btn-gold` PDF download (primary channel); POSTs to `/api/japa-guide`; loads `js/japa-guide.js`. Inline variant is the homepage `#japa-guide-band`. Canonical `/japa-guide` | No |
| `timeline-tool.html` | Dashboard iframe: Migration Timeline Planner — auto-opened by `js/autoopen-timeline.js` (`type=module`); no Stripe. Persistence via `js/timeline-persistence.js` (see Migration Timeline Persistence below): `openTimelineModal()` calls `hydrate()` and branches on three states — no session → read-only buffered render + sign-in nudge (never blank); session-not-Pro → posts `'initiate-pro-upgrade'` to `window.top`; session-Pro → editable, hydrated, lands on step 4 (generated timeline) when complete state exists | No (free with cap) |
...
| `news.html` | Immigration News Feed — indexable; full navbar + footer; `NewsMediaOrganization` JSON-LD; reads `/api/news`; filter pills (All/USA/UK) + visa-type `<select>` + opt-in Community pill; `js/newsfeed.js` (ES module, `createElement`-only render) + `js/news-interactions.js` (non-module IIFE, loaded after newsfeed.js). Phase 2 interaction layer LIVE (bookmark/react/share + personalised default filter). Phase 3 context trigger not built | No |
...
| `check/eb-2-niw.html` | Free EB-2 NIW Eligibility Checker (GEO cluster page 5) — indexable crawlable shell (H1 + About + FAQ) with an inline client-only tool mount below; SoftwareApplication + FAQPage + BreadcrumbList JSON-LD; client-only (no backend, no Supabase, no PII, no storage); `js/niw-checker*.js`; **band display gated by `NIW_CHECKER_BANDS_ENABLED=false`** | No |
...
├── assets/orabo-demo-loop.webm, orabo-demo-loop.mp4, orabo-demo-poster.webp  # Hero product-demo loop video (Phase 3) + webp poster
├── _headers                 # Cloudflare edge server-level HTTP headers — authoritative site-wide CSP
├── _redirects               # 47 permanent 301 redirects (.html → extensionless)
├── src/index.js             # Cloudflare Worker fetch handler (ES module; wrangler.jsonc `main`) — edge geo detection. Runs on EVERY request via assets.run_worker_first, serves the matched asset through env.ASSETS, then sets orabo_region=regulated|unregulated cookie (Path=/ Max-Age=43200 SameSite=Lax, not HttpOnly) + x-orabo-region header. Additive only: never rewrites body/status. Defaults unregulated + logs [orabo-region] console.warn when request.cf absent; throws if env.ASSETS unbound. Replaced the dead Pages functions/_middleware.js (the project is Workers-type, where the Pages _middleware convention is silently ignored at deploy)
...
│   ├── news.css             # News Feed (/news) — .news-card (official/community badge variants), .news-grid, .news-filter-bar (reuses .filter-row/.filter-btn from cards.css); Phase 2 control classes .news-card-actions/.news-reaction-bar/.news-react-btn(.is-active)/.news-bookmark-btn(.is-bookmarked)/.news-share-btn/.news-share-panel(.open)/.news-toast(.show); imported in styles.css after discovery-pages.css, before mobile.css
...
│   ├── cookie-banner.css    # Cookie consent banner + customise modal (F6.10.2) — .cookie-banner/.cookie-modal (.active visibility), .cookie-settings-link footer rule; imported in styles.css before mobile.css. links.html loads this directly (it has no styles.css)
│   ├── japa-guide.css       # Japa Readiness Guide lead magnet — .japa-guide-band (homepage navy wrapper) + .japa-guide-form card (shared by /japa-guide + homepage) + .japa-guide-success; reuses .btn/.btn-gold/.full-width + .hidden; imported in styles.css before mobile.css
...
│   ├── niw-checker.js / niw-checker-score.js / niw-checker-questions.js / autoopen-niw-checker.js  # Free EB-2 NIW Eligibility Checker (check/eb-2-niw.html); niw-checker-score.js is the PURE deterministic engine (tests/niw-score.test.js); window.niwChecker={init}; client-only, no storage; band display gated by exported NIW_CHECKER_BANDS_ENABLED
│   ├── universities.js / scholarships.js / opportunities.js / opp-teaser.js  # Discovery content modules
│   ├── newsfeed.js          # News Feed (/news) — fetches /api/news; createElement-only render (third-party RSS); timeAgo() relative-time helper; destination/visa/community filters + load-more + history.pushState. buildCard() emits data-news-id + an EMPTY .news-card-actions mount (owned/populated by news-interactions.js — keep empty here). Filter state is closure-locked; drive it externally only by clicking .filter-btn[data-dest] pills
│   ├── news-interactions.js # News Feed Phase 2 (non-module IIFE, loaded after newsfeed.js) — bookmark/react/share + GET /api/news/user-state hydration + GET /api/news/preference personalised default (pill-simulation). Event delegation on stable #news-grid + MutationObserver (re-injects controls after newsfeed.js re-renders) + immediate first-paint pass (Guard 1) + re-hydrate once user-state resolves (Guard 2). Inlines readSessionFromStorage (NEVER getSession). Logged-out clicks show a sign-in toast (.news-toast), not an error. Same-type reaction re-tap = DELETE /react (clear); switch = POST /react
│   ├── cookie-banner.js     # Cookie consent banner (F6.10.2) — ES module on all 45 footer-bearing pages; geo-gated on orabo_region cookie; localStorage cookie-consent-v1; exports + window.getCookieConsent()
│   ├── newsletter.js        # subscribeToDigest() + waitlistDigestHook()
│   ├── japa-guide.js        # Japa Readiness Guide lead magnet — ES module, wires every .japa-guide-form on /japa-guide + homepage #japa-guide-band; POSTs {email,destination,utm} to /api/japa-guide; success state = tappable .btn-gold PDF download (PRIMARY; auto-click best-effort only for in-app webviews); fires oraboTrack('japa_guide_signup',{destination,...utm}); loaded on index.html via main.js import + directly on japa-guide.html
...
│   ├── timeline.js / timeline-persistence.js  # Migration Timeline render engine + persistence layer (hydrate/persist/serialize; localStorage orabo_timeline_state buffer + /api/timeline source of truth)
...
│   ├── timeline.js      # GET/POST /api/timeline → Orabo Prod migration_timeline_state; JWT-gated (resolveAuthUser mirrors quiz.js); service-role upsert; rate-limited 30/min; logs [timeline]
...
│   ├── japa-guide.js    # POST /api/japa-guide — Japa Readiness Guide lead-magnet intake; Brevo upsert to BREVO_JAPA_LIST_ID + JAPA_DESTINATION attr; fire-and-forget backup email ([brevo-japa-guide]) with the PDF link; dedicated japaGuideLimiter; never touches Digest list/automations
│   ├── internal.js      # POST /api/internal/run-digest — x-digest-secret header
│   ├── me.js            # GET /api/me/export — GDPR Art 20 data export (JWT-only); mounted /api/me; see route spec below
│   └── auth-proxy.js    # POST /api/auth/signup — proxies Supabase's own signUp() and normalizes the response (new/existing-account signals stripped) before returning; account-enumeration hardening (Pass 9)
├── utils/
│   ├── verify-payment.js     # Payment gate for all AI routes
│   ├── auth.js               # getAuthenticatedUserId(req) + requireAuth middleware — shared JWT-via-Bearer-header resolver (Pass 9); used by routes/stripe.js
│   ├── rate-limit-key.js     # resolveClientIp(req) — shared express-rate-limit keyGenerator for all 12 limiters; trusts CF-Connecting-IP only when corroborated against Cloudflare's own ranges (Pass 9)
│   ├── cloudflare-ips.js     # isCloudflareIp(ip) — hardcoded Cloudflare edge CIDR ranges (IPv4 + IPv6) from cloudflare.com/ips (Pass 9)
│   ├── sanitize-body.js      # sanitizeBody middleware — strips embedded null bytes from every string in req.body, recursively (Pass 9)
...
- OPS tables `automation_runs`, `automation_alerts` — service-role writes only, RLS zero-policy deny-all. `seo_page_ledger` lives in the CONTENT DB (P1 A1, below).
...
### Autonomous Marketing Engine — A1 (Programmatic SEO)

First AME job that publishes to the public web autonomously. A GitHub Action in the frontend repo generates GEO-extractable calculator pages, commits them natively, and reports telemetry through the P0 `runJob` via a single token-gated backend ingest endpoint (no DB credentials in the Action). Scope is the two pure-math archetypes only: cost-of-living + take-home-pay (visa processing-time/difficulty pages are deferred to P1.5).

Backend (`japaconnect-backend`):
- `routes/ame-ingest.js` — mounted `POST /api/ame-ingest`, own `express.json({limit:'1mb'})` BEFORE the global 20kb parser. Fail-closed `x-ame-ingest-token` vs `process.env.AME_INGEST_TOKEN` (mirrors `routes/internal.js`; 401 on unset/mismatch). Records the batch via `runJob('seo_generate','event', …)` (OPS `automation_runs`; `records_out`=committed count, `partial` when any skipped), upserts `seo_page_ledger` rows (CONTENT `pg.Pool`, `on conflict (combination_key)`), and inserts one `automation_alerts` warn per lint-skipped page. Ledger upsert runs INSIDE the `runJob` fn so a CONTENT failure becomes a failed run + error alert. Intentional cross-DB route (OPS via `runJob` + CONTENT via `pool`; two independent writes, no JOIN).
- `migrations/2026-07-03-seo-page-ledger-content.sql` — CONTENT DB (`fcogjoxcfmrcuvtreivm`). `seo_page_ledger` (`combination_key` unique, `archetype` ∈ {col,salary}, `url`, `status`, `african_first`, `batch_ref`, `records_out`, `first_committed_at`, `last_verified_at`). RLS zero-policy; the pool owner role has full access (no `service_role` GRANT). Apply manually in the CONTENT SQL editor.
- New env: `AME_INGEST_TOKEN` (Railway backend env + frontend-repo GitHub Actions secret).

Frontend (`japaconnect`):
- `scripts/generate-pseo-calculators.mjs` + `scripts/pseo-lib/` (`slug`, `data`, `chrome`, `geo-lint`, `template-col`, `template-salary`, `sitemap-redirects`) — emits `/cost-of-living/<a>-vs-<b>` + `/take-home-pay/<role>-<country>` pages from the live datasets (single source of truth via the `-data.js` modules). Flags: `--archetypes` `--limit`(12) `--dry-run` `--inject-lint-canary`. African-first ordered; col/salary interleaved so every batch mixes both. Idempotency: filesystem-authoritative existence check + canonical `combination_key` (reverse-duplicate collapse). Sitemap + `_redirects` updated per-URL idempotently (NOT the compare generator's short-circuit).
...
1. Sign Up → `signup.html` → `supabase.auth.signUp()` → verification email. `js/signup.js` then calls `POST /api/profile/create` so the backend creates the `user_profiles` row (client-side insert is impossible pre-confirmation under RLS; see Integrations → Orabo Prod).
...
- **Tables:** `user_profiles`, `purchases`, `one_off_purchases`, `ai_cache`, `processed_webhook_events`, `analytics_events`, `budget_tracker`, `timeline_milestones`, `migration_timeline_state`
...
- `user_profiles` columns: `id`, `email`, `full_name`, `country_origin`, `pro_status`, `pro_expires_at`, `stripe_customer_id`, `stripe_subscription_id`, `quiz_results` (jsonb), `user_preferences` (jsonb), `journey_state` (jsonb), `role` (admin gate), `pending_deletion`, `deletion_requested_at`, `deleted_at`. **Canonical destination** = `user_preferences.destination` (jsonb key) with a `user_preferences.destination_source` sibling (`explicit`|`implicit`); the legacy `destination_country`, `pathway`, and `target_arrival_date` columns exist in the OPS table but are **deprecated and unwritten**. **Column-level write lockdown (`trg_user_profiles_column_lockdown`, `migrations/2026-07-17-user-profiles-column-lockdown.sql`, backend repo):** RLS on this table is row-level only (`auth.uid() = id`) — it does not restrict which columns an authenticated user can change on their own row. A `BEFORE INSERT OR UPDATE` trigger closes that gap: for the `authenticated` role only (detected via `auth.role() = 'service_role'` early-return), every column except `full_name`/`country_origin` is forced back to `OLD.*` (UPDATE) or `NULL`/default (INSERT). Extending which columns a user may self-edit requires updating this trigger, not just a new frontend call. The row is created at signup time by the backend, NOT a client-side insert. `js/signup.js` only CALLS the endpoint: it fires `POST /api/profile/create` with `{ userId, email, name, country_origin }`; the WRITE happens in `routes/profile.js` (mounted at `/api/profile`, OPS service-role client, dedicated `profileLimiter` 30/hr/IP, console prefix `[profile-create]`), which does an idempotent `upsert({ id, email, full_name, country_origin }, { onConflict:'id', ignoreDuplicates:true })` and sets no privileged columns (`pro_status` takes its schema default of `false`). **Why backend-side:** the original client-side insert always failed silently under RLS. Email confirmation is ON, so `signUp` returns no active session, `auth.uid()` is NULL, and the INSERT policy `user_profiles_insert_own` (`WITH CHECK auth.uid() = id`) denied it (PostgREST `42501`); supabase-js returned `{ error }` (logged, not thrown), so the UI proceeded but no row was created. The canonical `auth.users` `handle_new_user` `SECURITY DEFINER` trigger fix is NOT usable here either: the SQL-editor role cannot own or ALTER triggers on `auth.users` (`42501 "must be owner of table users"`), so the trigger landed disabled and was dropped. Do NOT re-attempt the `auth.users` trigger approach. **Security:** the endpoint resolves `email` server-side from `auth.users` via `getUserById(userId)` (400 if the user doesn't exist) and ignores the client-supplied body email, closing the IDOR/email-poisoning vector; `name`/`country_origin` stay client-supplied (cosmetic) and are protected from overwrite by `ignoreDuplicates`. Never set `pro_status`/`stripe_*`/`role` via this endpoint. For any backfill the canonical email source is auth `auth.users` via `supabase.auth.admin.listUsers()`, not this table; `scripts/backfill-user-profiles.js` (service-role, idempotent upsert) was run once to create rows for the 33 pre-existing profile-less users (all 33 now have rows) and is safe to re-run (would upsert 0).
- **Destination canonical store:** `user_preferences.destination` (jsonb), normalised via backend `utils/destination.js` `NORMALIZE_DESTINATION` (ten-token controlled set: `USA, UK, Canada, Germany, Ireland, Australia, NewZealand, Netherlands, Portugal, Other` — exact casing, the News Feed read path depends on it). Written **service-role only** via `utils/write-destination.js` `writeDestination({jwt, rawDestination, source})` or the JWT-gated `POST /api/profile/destination` route (always explicit tier; also accepts `{ dismissed: true }` → `dismissDestinationPrompt({jwt})` which sets `user_preferences.destination_prompt_dismissed` sibling-preserving, never touching destination). Precedence: explicit > implicit with recency within tier, tracked by the `destination_source` marker (`explicit`|`implicit`); implicit never overwrites explicit. `destination_country` column deprecated/unwritten — never write it. **Wired explicit-tier callers:** `routes/checklist.js` (raw `req.body.country`) and `routes/report.js` (raw `req.body.destination`) call `writeDestination(…, source:'explicit')` fire-and-forget (`.catch`-logged `[destination-capture] <surface> write failed`) at both the cache-hit and fresh-generation success exits, JWT-guarded on `req.headers.authorization`. **Coverage is Pro-authenticated runs only** — the free and payment-return callers send no JWT, so only the `js/pro-bypass-checklist.js` / `js/pro-bypass-quiz.js` Bearer path captures (accepted D6 gap). CV/SOP are unwired (standalone anonymous pages send no JWT). **Dashboard prompt wired (Phase D):** `js/dashboard-destination-prompt.js` (non-module, loaded after `dashboard-inline.js`; `window.initDestinationPrompt(user_preferences)` called from `initDashboard()` after the profile fetch) renders the quiet journey-panel prompt `#dj-dest-prompt` ("Where are you headed?") ONLY when `user_preferences.destination` empty AND `destination_prompt_dismissed` not set. One-tap chips (USA/UK/Canada + "Somewhere else" → inline `#dj-dest-expansion` long tail) POST the raw token to the route; the × posts `{ dismissed: true }` (durable). Reads `user_preferences` from the already-fetched profile (no second fetch); fail-closed CSS `.dj-dest-prompt`/`.visible` in `css/dashboard-page.css`. Implicit-tier (EB/O-1A/UKGT → USA/UK) is deferred — these tools are anonymous Stripe-session flows (no Supabase UMD, no pro-bypass, not dashboard iframes), so no `Authorization: Bearer` JWT reaches their `/api/eb1a`, `/api/eb2`, `/api/o1a`, `/api/ukgt-tech` generation moments; with no authenticated surface, an implicit write would always no-op (D6 gap). Wire only if/when an authenticated caller for these routes exists. Pass the raw dropdown string at call sites — never pre-normalise (`NORMALIZE_DESTINATION` inside the helper is the single authority).
- `one_off_purchases` columns: `id`, `name`, `email`, `tool`, `stripe_session_id` (nullable), `created_at`.
- `budget_tracker`: Live user-PII OPS table (class A, `user_id` FK → `user_profiles.id`) per data map v1.3 §1; hard-deleted by `user_id` in the F6.10.3 purge and enumerated by the F6.10.4 export. Flagged orphan risk (absent from the original brief); audit before reuse.
- `timeline_milestones`: Live user-PII OPS table (class A, `user_id` FK → `user_profiles.id`) per data map v1.3 §1; hard-deleted by `user_id` in the F6.10.3 purge and enumerated by the F6.10.4 export. Flagged orphan risk (absent from the original brief); audit before reuse. NOT the live Migration Timeline store — that is `migration_timeline_state` (below). The orphaned `js/dashboard.js saveTimelineMilestones()` wrote this table via the old browser-direct RLS path; it is dead (imported by no HTML) and slated for removal.
- `migration_timeline_state`: Live persistence for the Migration Timeline Pro feature. One row per user, PK `auth_user_id` (FK `auth.users(id) on delete cascade`), single `state` jsonb (`{selectedPath, targetDate:'YYYY-MM', checkedPrep:string[]}`), `updated_at`. Written ONLY by the backend `/api/timeline` service-role route; RLS own-row policies + `service_role` GRANT (migration `2026-06-23-migration-timeline-state.sql`). Reconciliation last-write-wins by `updated_at`. Enumerated by the F6.10.4 export (keyed `auth_user_id`).
...
- **Digest auto-send is LIVE** — the watched first production digest was verified 2026-06-26 (33 sent / 32 delivered / 0 errors / 0 duplicate `digest_sends` ledger rows), so BOTH Saturday 08:00 UTC triggers are now active: the GitHub Action `schedule:` block in `.github/workflows/weekly-digest.yml` and the `scheduler.js` digest `cron.schedule('0 8 * * 6', …)` registration. The pair is redundant by design and idempotent per-recipient via the `digest_sends` ledger (whichever fires first wins; the other `skipped_already`). The route, its secret-gating, and the `?testEmail=` / `?dryRun=` manual paths remain fully live.
...
- Required Brevo custom attributes: `DESTINATION` (text), `QUIZ_SCORE` (number), `QUIZ_DATE` (text), `QUIZ_LEAD` (text), `PURCHASE_DATE` (date), `LAST_TOOL` (text), `LAST_PREVIEW_TOOL` (text), `LAST_PREVIEW_DATE` (date), `WORTHIT_TARGET` (text), `WORTHIT_VERDICT` (text), `PREF_PATHWAY` (text), `PREF_FIELD` (text), `JAPA_DESTINATION` (text — Japa Readiness Guide funnel; set ONLY by `routes/japa-guide.js`, distinct from `DESTINATION`; created via `scripts/setup-japa-brevo.mjs`).
- `sendDigest({ force, test, dryRun, userId, returnHtml, onlyEmail })`: never throws to caller. Idempotency is per-recipient via the `digest_sends` OPS ledger (`recordSend` UPSERTs `{ email, user_id, week_key, message_id, delivery_status:'accepted', reconciled_at:null, sent_at }` on the `(email, week_key)` unique key AFTER each successful send — at-least-once; upsert (not insert) so a re-sent transient-blocked recipient resets to a fresh `accepted` state with the new messageId rather than orphaning it on the stale row; the loop skips any `(email, week_key)` whose `delivery_status IN ('accepted','delivered')` unless `force`). At run-end the full-list path (NOT `onlyEmail`/`test`/`dryRun`) emails ONE batched per-run admin summary via `sendAdminNotification` (context `digest-summary`, subject `Digest <week>: <sent> sent / <delivered> delivered / <n> hard-bounced`) — provisional acceptance numbers, since bounces reconcile asynchronously after the run. `test=true` builds + logs counts, sends nothing, writes no ledger row. `force=true` bypasses the already-sent skip. `dryRun=true` + `userId` builds for ONE user and sends only to them (or returns HTML with `returnHtml=true`) — **no ledger write, no blacklist check** (preview path only). `onlyEmail=<addr>` scopes the REAL list loop to a single Brevo list contact — exercises the identical ledger + idempotency + blacklist path on one address (watched first-run verification); address must already be a list contact. The three guards (ledger write, `!force && alreadySent` skip, `emailBlacklisted` skip) live ONLY in the full-list loop, so `onlyEmail` — not `dryRun` — is the path that verifies them. Summary returns `{ sent, skipped_already, skipped_blacklisted, errors, week_key }`.
- `POST /api/internal/run-digest` — `x-digest-secret` header (`DIGEST_TRIGGER_SECRET`); `?force=true` + `?test=true`; `?dryRun=1&userId=<id>[&html=1]` (inline single-recipient preview/send); `?testEmail=<addr>` (inline single-recipient REAL send through the canonical loop, returns the summary). Full-list run (no `testEmail`/`dryRun`) background-dispatches and returns `{ dispatched: true }` immediately. Never referenced in frontend. **Plus-addresses in `?testEmail=` must be percent-encoded (`+` → `%2B`)** — a literal `+` in the query string decodes to a space and fails to match any list contact (returns `only-email-not-on-list`, sends nothing).
- **Digest delivery is reconciled by the Brevo delivery/bounce webhook (`routes/brevo-webhook.js`, `POST /api/brevo-webhook?secret=<BREVO_WEBHOOK_SECRET>`).** `sendTransactionalEmail` (`utils/brevo-email.js`) treats any axios-resolved `2xx` from `/smtp/email` as ACCEPTANCE (logs `[brevo-send] accepted messageId=… to=…`) and now exposes the messageId to callers via `res.oraboMessageId` (property-attach — return shape unchanged, non-breaking). The digest loop stores that messageId on the `digest_sends` row (`message_id` column) at send time; `delivery_status` starts `'accepted'`. Because acceptance ≠ delivery (blocks/hard-bounces resolve asynchronously and never surface in the sync response), the webhook reconciles each row: mounted AFTER `express.json` (Brevo posts JSON, no raw-body/HMAC — unlike Stripe); URL query-secret auth (`req.query.secret === BREVO_WEBHOOK_SECRET`, mirrors `routes/internal.js` posture); **fail-open** (401 ONLY on bad/missing secret, every other outcome returns 200 to avoid Brevo retry storms). It resolves the target row by Brevo's hyphenated `message-id` (falls back to `email` + week-of-event only when absent) and writes `delivery_status` + `reconciled_at`: `hard_bounce`/`spam` win **unconditionally** (terminal-negative — permanently suppress); `delivered` and transient `blocked`/`soft_bounce`/`deferred` use a **guarded** UPDATE that never regresses a terminal-negative row. The digest **skip predicate counts only `delivery_status IN ('accepted','delivered')` as suppressing**, so a transient block/soft-bounce becomes re-eligible on the next run while `hard_bounce`/`spam` stay terminal-suppressed. Idempotent by construction (terminal-negative last-write-wins; `delivered`/transient guarded) — no `processed_webhook_events`-style ledger needed. Reconciliation is async, so the run-end admin summary reflects acceptance and firms up minutes-to-hours later (a delayed reconciliation summary is a possible follow-up). Webhook URL (with `?secret=`) must be registered in the Brevo dashboard transactional webhook settings — a manual step (like the Stripe `charge.refunded` subscription). Schema added by `migrations/2026-06-26-digest-delivery-status-ops.sql` (`message_id`, `delivery_status` DEFAULT `'accepted'`, `reconciled_at`, `idx_digest_sends_message_id`, `GRANT UPDATE … TO service_role`).
- Env: `BREVO_API_KEY`, `BREVO_LIST_ID`, `BREVO_SENDER_EMAIL`, `BREVO_SENDER_NAME`, `ADMIN_NOTIFY_EMAIL`, `DIGEST_TRIGGER_SECRET`, `BREVO_WEBHOOK_SECRET` (URL query-secret gating `POST /api/brevo-webhook`), `BREVO_JAPA_LIST_ID` (numeric id of the "Japa Readiness" list — required by `routes/japa-guide.js`), `BREVO_JAPA_SENDER_EMAIL` (optional; the Japa Guide backup email sender, defaults to `hello@orabo.app` in code — must be a verified Brevo sender).
- **Japa Readiness Guide lead magnet (`POST /api/japa-guide`, `routes/japa-guide.js`):** dedicated intake, isolated from the Weekly Digest list + its automations. Upserts the contact onto the `BREVO_JAPA_LIST_ID` list (`updateEnabled:true`, 201=new / 400 `duplicate_parameter`=existing) with the `JAPA_DESTINATION` attribute, then fires a fire-and-forget backup email (`_sendGuideEmail`, `[brevo-japa-guide]`) from `hello@orabo.app` carrying the PDF download **LINK** `https://orabo.app/downloads/japa-readiness-guide.pdf` (never an attachment) — sent on every successful submit so it always arrives. A Brevo contact failure is logged but never fails the response (the on-page download is the primary channel and is never gated). Dedicated `japaGuideLimiter` (env `JAPA_GUIDE_RATE_LIMIT_MAX` default 10/hr/IP). Destination allowlist = the 7 homepage values verbatim. `scripts/setup-japa-brevo.mjs` = one-time list_id lookup + attribute creation; `scripts/clean-japa-test-contacts.mjs` = dry-run-by-default test/QA contact removal before broadcast.
...
- Tracked events: `homepage_primary_cta_click`, `tool_started` (props: `tool_name`), `checkout_initiated` (props: `item_key`), `checkout_succeeded` (props: `item_key`), `checkout_abandoned` (props: `item_key`), `scroll_depth` (props: `depth_percent`), `japa_guide_signup` (props: `destination` + `utm_source`/`utm_medium`/`utm_campaign`/`utm_content`/`utm_term`; fired from `js/japa-guide.js` on lead-magnet submit success; insight `V4jpAyOA` "Japa Guide signups by destination (30d)" pinned to dashboard `746213`).
...
- `V4jpAyOA` — Japa Guide signups by destination (30d). Bar chart on `japa_guide_signup` broken down by `destination`. Lead-magnet capture volume + which destinations convert; UTM props ride along on the event. Populates once the funnel sees real traffic.
...
- **paycalc-data.js pattern:** `ROLES[]`, `ROLE_GROUPS`, `SALARY_DATA{}`, `COUNTRIES` (fxRate/taxRate/cityId), `COUNTRY_GROUPS`, `COL_MONTHLY_USD`, `NGN_RATE` all export from `js/paycalc-data.js` (Node-safe, no `window`/`document`); `paycalc.js` imports them back. The tax table (`COUNTRIES`/`COL_MONTHLY_USD`) was moved out of `paycalc.js` so the programmatic-SEO generator (AME A1) computes net pay from the same source the live tool uses. Adding a role: add to `ROLES`/`ROLE_GROUPS`/`SALARY_DATA` for all 18 countries. Adding a country: `SALARY_DATA` key for all 35 roles + `COUNTRIES` entry (fxRate, taxRate, cityId) + `COL_MONTHLY_USD` + `COUNTRY_GROUPS` array + `COUNTRY_DATA` in `all-countries-pay-app.js` (still a separate duplicate).
...
- **CSP is enforced via `_headers` (Cloudflare edge) — not meta tags.** `_headers` in the repo root is the single authoritative CSP. All per-page `<meta http-equiv="Content-Security-Policy">` tags removed. When adding a new external domain dependency, update `_headers` only. Never re-add meta CSP tags.
...
- **Cookie consent (F6.10.2):** geo-conditioned via the Worker fetch handler `src/index.js` (CF Workers edge; `assets.run_worker_first=true` so it runs on every request, then serves the asset via `env.ASSETS`): reads `request.cf.country`/`regionCode`, sets `orabo_region=regulated|unregulated` cookie + `x-orabo-region` header. Regulated = EU27 + EEA (IS LI NO) + GB + BR + US-California (`country==='US' && regionCode==='CA'`). `js/cookie-banner.js` renders the banner only when `orabo_region=regulated` AND no valid stored choice; footer `[data-action="cookie-settings"]` link opens the customise modal for everyone regardless of region/choice. Consent in `localStorage.cookie-consent-v1` `{essential,analytics,marketing,setAt(ISO),version:'v1'}`, 12-month expiry; bump `version` to invalidate. Banner z-index 9990 (below sticky CTA 9999), modal 10000. `.active`-class visibility, zero inline styles/handlers (CSP). `window.getCookieConsent()` returns `{essential,analytics,marketing}` (defaults all-off when unset). **Analytics binding:** `js/visitors.js trackVisit()` suppresses only on EXPLICIT decline (stored `analytics===false`); absence of a stored choice (the unbanner'd global majority) still tracks. No Brevo tracking pixels exist, so `marketing` has no v1 binding. The footer "Cookie Settings" link lives on all 45 footer-bearing pages alongside Privacy/Terms — add it (and the `<script type="module" src="/js/cookie-banner.js">` tag) to every new footer page; the 13 `/compare/*` pages come from `scripts/generate-compare-seo.mjs buildFooter()` (edit + re-run, never patch statics).
...
- **Data freshness governance:** every static data file displaying numbers exports `DATA_FRESHNESS` `{ source, verified, cadence_days, tier }`. Tiers: `fx` (30d), `fees` (90d), `costliving` (90d), `eligibility` (365d). Run `node scripts/check-data-freshness.mjs`. `js/costliving-data.js` (Node-safe, no `window`/`document`) holds `FX_RATES`, `FX_BASELINE`, `FX_SYMBOLS`, `CITIES`, `COST_DATA`, `LINE_ITEMS`, `DATA_FRESHNESS`; `costliving.js` imports them back (the city data moved there so the programmatic-SEO generator reads the same source). See `FRESHNESS.md`.
- **Programmatic-SEO generator (AME A1) — `scripts/generate-pseo-calculators.mjs` + `scripts/pseo-lib/`:** emits GREEN-LIST content ONLY (cost-of-living + take-home-pay arithmetic; ZERO legal, eligibility, or immigration claims). Every page passes the per-page fail-closed lint+GEO gate in `scripts/pseo-lib/geo-lint.mjs` (em-dash / brand-leak `japaconnect`|"Japa Readiness" / American-spelling in visible prose; single non-empty `<h1>`; ≥40-word lead paragraph; exactly one JSON-LD `@graph` of the 3 types [Article, FAQPage, BreadcrumbList] with `image`; `data-label` on every `<td>`; `<details>` count === schema `mainEntity` === rendered `.guide-faq-answer` with byte-sync; EXACT tool-CTA href) — a failing page is skipped + raises an `automation_alerts` warn, never published, never aborts the batch. Idempotency is filesystem-authoritative (skip if the `.html` already exists) plus canonical `combination_key` (reverse-duplicate collapse). Rate-limited (`--limit`, default 12). British English; zero em dashes. Extend the template, never emit a legal claim; if the template would emit one, stop. Verify with `node --test tests/pseo-lint.test.mjs tests/pseo-slug.test.mjs` + `--dry-run` before any live commit.
...
- **GEO checker tools (`/check/*`, greenfield route family; first member `check/eb-2-niw.html`):** indexable hand-authored shell (H1 + 2-sentence description + About + FAQ `<details>`) for citability + to end the guide-CTA 404, with an inline client-only tool mounted below; chrome modelled on `guides/*.html` (load `/styles.css` for `:root` tokens + footer/cookie chrome, page CSS in an inline `<style>` block, guide footer with Privacy/Terms/Cookie Settings + parent-org line, load `js/analytics.js` + `js/cookie-banner.js`). Inline JSON-LD: `SoftwareApplication` (incl. `"image": og-image.jpg`, `offers` price 0) + `FAQPage` (mirrors the visible FAQ) + `BreadcrumbList`. Tool follows the cvtool pattern: `window.<tool>={init}` + external `autoopen-*.js` poll (100ms×50); `.active`-class `showScreen(id)` toggling, never `style="display:.."`; all events via `addEventListener`. **Client-only contract:** answers live in memory for the session only — no sessionStorage/localStorage/backend/Supabase/PII; `oraboTrack` events are PII-free. **Scoring-display gate:** the pure deterministic engine (`*-score.js`, unit-tested under `tests/`) always computes, but band/score *display* is gated behind a fail-closed exported flag (`NIW_CHECKER_BANDS_ENABLED=false`) until attorney sign-off — flag-off renders the gap analysis + a "scoring being finalised" notice and suppresses overall band, prong bars, and band labels; the binary EB-2 gate-failure message (which names the specific threshold gap) still renders flag-off. Never show a percentage/probability or any "approval" language. **Results-CTA routing (`renderResults()` in `js/niw-checker.js`)** is two-layered: Layer 1 is gate-based and flag-independent (always LIVE) — a `gate === 'not_met'` result renders the alt-pathway CTA (`renderAltPathwayCta()`: primary `/pathway-compare`, secondary `/o1a-visa` + `/guides/eb-2-niw-without-phd`), NEVER a paid consult as primary (an attorney cannot fix a qualifications gap). Layer 2 is band-based and renders ONLY when `NIW_CHECKER_BANDS_ENABLED === true` — all-Competitive → `renderEbToolsCta()` (`/eb2-package-tool.html` + `/rfe-response` + consult), any Developing/Emerging prong → `renderGapGuidesCta()` (guide + compare + soft petition-tools, no paid lead). While the flag is false the gate-passed CTA is always `renderGenericCta()` (consult + guide) regardless of band, so a hidden band cannot be inferred from the button shown. CTAs use only `.niw-cta-block`/`.niw-cta-primary`/`.niw-cta-secondary` (no new CSS). `checker_result_cta_clicked.target` ∈ `eb_tools | attorney_consult | gap_guides | alt_pathways` (PII-free). Add the `_redirects` 301 (`/check/X.html → /check/X`) + a sitemap `<url>` (priority 0.7) per new checker. Node v22+ auto-detects ESM in `.js`, so `node --test tests/*.test.js` imports `js/*-score.js` with no `package.json`.
...
- **Data export (`routes/me.js`, GET /api/me/export, mounted `/api/me`):** GDPR Art 20 export. JWT-only (no Stripe-session fallback): missing/empty `Authorization: Bearer` → 401 `{error:'auth_required'}`; `supabase.auth.getUser(jwt)` failure → 401 `{error:'auth_invalid'}`. The resolved `user.email` is the canonical join key for all email-keyed tables. In-memory `Map<auth_user_id,timestamp>` rate limit 1/min/user (429 + `Retry-After`; 5-min prune; becomes Redis-backed if the backend scales horizontally). Authority for the table walk is `audit/privacy-data-map-v1.2.md @ 08ae25e` (in the BACKEND repo) — NOT the brief or this file. Walks every (A)-classified table: OPS via Supabase service-role JS client (`user_profiles` by id; `purchases`/`checklist_items`/`eligibility_results`/`budget_tracker`/`timeline_milestones` by user_id; `migration_timeline_state` by **`auth_user_id`** (its PK column, not `user_id`); `one_off_purchases` by email), CONTENT via `pg.Pool` (`post_earn_rewards`/`stories`/`opportunities` by email, ALL moderation states — never filter `approved=true`). `budget_tracker`+`timeline_milestones` are live user-PII, enumerated/purged by `user_id` (data map v1.2 §1). Phase-2 tables (`news_bookmarks`,`news_reactions`) guarded by relation-missing probe (PostgREST exposes neither `to_regclass` nor `information_schema`, so detect error codes `42P01`/`PGRST205` and omit the key). Out-of-scope per the map (deliberately NOT exported): `analytics_events` (Art 11), `ai_cache`, `news_items`, `processed_webhook_events`. Stripe (`customers.retrieve` + `paymentIntents.list` limit 100) + Brevo (`GET /v3/contacts/<email>`, 404 → `null`) snapshots fetched in parallel with graceful degradation (`{error:'snapshot_unavailable'}` + `export_metadata.partial=true`). Response carries `Content-Disposition: attachment`. Log prefix `[me-export]`. Dashboard control: `js/dashboard-privacy.js` (non-module, loaded after `dashboard-inline.js`; reuses global `showToast` + `window.ORABO_API_URL`; reads JWT via localStorage scan) wires `#privacy-export-btn` in the `#dash-privacy-panel` inside the journey home.
- **Migration Timeline Persistence (`js/timeline-persistence.js` + `routes/timeline.js` + table `migration_timeline_state`):** `js/timeline.js` stays render-focused and calls the persistence layer. State shape `{ selectedPath: string, targetDate: 'YYYY-MM' string, checkedPrep: Set }`; `step` is NOT persisted. **CRITICAL serialization rule:** `checkedPrep` is a `Set` — `serializeState()` MUST `Array.from()` it (a Set JSON-stringifies to `{}`, silently losing all prep). `deserializeState()` rehydrates back to a Set and keeps `targetDate` raw (the `-01` append stays at compute time in `timeline.js`). Persistence module inlines `readSessionFromStorage()` verbatim (returns `{ access_token, user }`) — NEVER `getSession()`/`getUser()` inside the iframe (dual-GoTrueClient deadlock). localStorage buffer key `orabo_timeline_state` (offline/fallback only, NOT a parallel datastore); Supabase OPS `migration_timeline_state` is source of truth. `hydrate()` reconciles last-write-wins by `updated_at` and returns `{ state, mode }` (`'unauth'` when no session). `persist(state)` = synchronous `saveLocal` + debounced (`~900ms`) `saveRemote`; failures never disrupt render. `openTimelineModal()` branches: no session → read-only buffered render (`tl-readonly` class, inert controls, `.tl-signin-nudge`) NEVER blank; session-not-Pro → `postMessage('initiate-pro-upgrade', window.top)`; session-Pro → editable + hydrated, lands on step 4 when complete. The read-only flag (`readOnly`) guards every `addEventListener` block and `persist()` call. Do NOT revive the orphaned `js/dashboard.js saveTimelineMilestones()` / `timeline_milestones` browser-direct path.
...
### High — needs Supabase/DNS dashboard access (from Pass 9, not resolvable from either repo)
- **M-01:** Supabase dashboard → Settings → API → restrict CORS to `https://orabo.app` + `https://www.orabo.app` only. Verify: `curl -D- <supabase-url>/rest/v1/user_profiles?select=id&limit=1 -H "apikey: <anon>" -H "Origin: https://evil.com"` should not echo `https://evil.com` back in `access-control-allow-origin`.
- **H-02:** no code path exists to rate-limit `/auth/v1/token` — `signInWithPassword` calls Supabase directly from the browser, never touching the Express backend. Needs a Supabase Auth dashboard rate-limit/CAPTCHA setting and/or a Cloudflare WAF rule.
- **C-08 full closure:** check the Supabase Auth dashboard for any setting controlling whether `/auth/v1/signup` differentiates existing vs. new email in its response body (`identities`/`role` fields). If found, enable it and re-verify via `curl` against the raw endpoint. If not found, C-08 stays a partial mitigation (our own signup UI is fixed via `routes/auth-proxy.js`; the raw Supabase REST endpoint is not) — that status needs to be communicated as such, not silently upgraded to "closed."
- **M-03:** DNS — SPF `~all` → `-all`; DMARC `p=none` → `p=quarantine`; configure DKIM via Brevo.

### Medium — deferred by design (Pass 9, needs a product/scope decision, not a quick patch)
- **L-07:** `POST /api/newsletter/subscribe` accepts any email with no ownership verification — someone can subscribe a third party without consent. Real fix is double opt-in, which adds signup friction; needs a product call before implementing.
- **L-01:** removing `unsafe-inline` from `_headers`' `style-src` requires converting 172+ `element.style.display/.overflow =` call sites (30 JS files) to CSS classes, plus a nonce carve-out for Chart.js. Multi-day project — track separately, don't fold into a closeout pass.
- **L-02 full audit:** `admin.js`/`dashboard-inline.js` were spot-checked and look clean (Pass 3's `esc()` discipline holds), but a file-by-file pass over all 61 files using `innerHTML` wasn't completed. Low priority unless a specific file is flagged.

### CLOSED 2026-07-17 — Pass 9 pre-re-test closeout (remaining external report items)

Follow-up to Pass 8: closed or investigated every item still open on the external access-control report ahead of a possible re-test invitation. Status per item:

- **H-04 (fixed, and worse than reported):** `POST /api/create-portal-session` had no auth check at all, not just the wrong status code — any caller could pass an arbitrary `userId` and receive a live Stripe Billing Portal URL for that other user's account. Fixed by `utils/auth.js` (new shared `getAuthenticatedUserId`/`requireAuth`), used in `routes/stripe.js`: `createPortalSession` now derives identity from the JWT only (ignores `req.body.userId`); `createCheckoutSession` does the same for `mode: 'subscription'` (one-time/anonymous tool purchases are unaffected, that flow is intentionally unauthenticated). Frontend `js/stripe.js` (`subscribeToPro`) and `js/auth.js` (billing-portal link, which was also silently broken — it POSTed `customerId` while the backend read `userId`) now send `Authorization: Bearer <token>` via `readSessionFromStorage()`.
- **M-05 (fixed, with a real caveat stated up front):** every rate limiter (12 total — 5 in `server.js`, plus `timelineLimiter`/`dashboardLimiter`/`quizSaveLimiter`/`previewLimiter`/`worthitLimiter`/`japaGuideLimiter`/`opportunitySubmitLimiter`, which the original research pass missed) keyed on raw `req.ip`, which the client→Cloudflare→Railway→Express hop count most likely resolved to Cloudflare's own edge IP, not the real client. New `utils/rate-limit-key.js` (`resolveClientIp`) + `utils/cloudflare-ips.js` (`isCloudflareIp`, hardcoded ranges from cloudflare.com/ips) trust `CF-Connecting-IP` only when the immediate socket peer is itself a Cloudflare address; otherwise falls back to `req.ip`. **Confirmed live 2026-07-17 that the raw Railway URL is directly reachable** (`curl` returns 200, bypassing Cloudflare entirely), so blind header-trust would have been fully bypassable — this was caught and fixed before ship, not after. Whether the fix achieves full precision depends on Railway's internal proxy topology (unconfirmable from either repo); worst case it's a no-op-safe fallback to current behavior, never a regression.
- **C-08 (partially mitigated, not fully closed — say so plainly):** confirmed real via direct `curl` against `https://rrkkbnoqubfmhrnxgngp.supabase.co/auth/v1/signup` — Supabase already avoids a loud error (both existing and new emails return HTTP 200), but the response body still differs (`identities: []` + `role: ""` for an existing account vs. a populated `identities` array + `role: "authenticated"` for a new one). New `routes/auth-proxy.js` (`POST /api/auth/signup`) calls Supabase's own `signUp()` server-side and always returns a fixed `{success, userId}` shape regardless of which case it was, closing the signal for our own UI (`js/signup.js` now calls the proxy instead of `supabaseClient.auth.signUp()` directly). **This does not close the vector against someone bypassing our frontend and hitting Supabase's REST endpoint directly with the public anon key** — that's Supabase's own public API, not interceptable from our side. No Supabase dashboard access was available to check for a platform-level suppression setting; report this to the external reporter as a partial mitigation, not a full close, if he re-tests.
- **M-04 (downgraded, not real as reported):** `routes/track.js`'s `path` field is only ever stored as a string in `analytics_events` — no code path fetches it server-side, so this isn't SSRF. Added lightweight validation anyway (`path` must start with `/`, no `://`) as defense-in-depth.
- **L-05 (fixed):** new `utils/sanitize-body.js` (`sanitizeBody`) strips embedded null bytes from every string in `req.body`, recursively. Wired in `server.js` after all four `express.json()` mounts (`/api/audit`, `/api/rfe`, `/api/ame-ingest`, and the global one) since the three raised-body-limit routes bypass the global parser.
- **L-03 (fixed):** `orabo_region` cookie (`src/index.js`, Cloudflare Worker) gained `Secure`. `SameSite=Lax` and no `HttpOnly` were already correct (the cookie banner reads it client-side). Backend sets no cookies of its own.
- **M-02 (fixed as code, not dashboard config):** `_headers` (repo root, edge-enforced on this Workers Static Assets deployment) gained `Strict-Transport-Security: max-age=31536000; includeSubDomains; preload`.
- **L-04 (fixed):** `@supabase/supabase-js` pinned from a floating `@2` jsdelivr tag to the exact version already used server-side (`2.103.3`), plus `integrity`/`crossorigin="anonymous"` added to all 8 supabase-js tags, the 3 `html2pdf.js@0.10.1` tags, and the 1 `chart.js@4.4.4` tag (12 total `<script>` tags across 10 HTML files).
- **L-06 (non-issue, no action):** Brevo's `updateEnabled: true` + existing `duplicate_parameter` handling already makes newsletter resubscription idempotent.
- **H-01 (closed, expected behavior):** both logout paths call `supabase.auth.signOut()` (`dashboard-inline.js` uses `{ scope: 'global' }`, revoking refresh tokens everywhere); ~1hr access-token lifetime is a Supabase platform default with no in-repo override.
- **M-06 (closed, moot):** `trg_user_profiles_column_lockdown` (Pass 8) already reverts every sensitive column on every authenticated write regardless of race timing; only `full_name`/`country_origin` are mutable and those are non-sensitive.
- **L-07, L-01, L-02 full audit, M-01, M-03, H-02:** still open — see the two "Pending" sections directly above (dashboard/DNS access and product-scope decisions this pass couldn't resolve).

New backend files: `utils/auth.js`, `utils/rate-limit-key.js`, `utils/cloudflare-ips.js`, `utils/sanitize-body.js`, `routes/auth-proxy.js` (mounted `/api/auth`).

### CLOSED 2026-07-17 — Pass 8 external access-control report response
- An external, unverified report claimed a full privilege-escalation chain (self-granted admin/Pro, data exposure, exposed `.git`). Archaeology found the report's own diagnosis wrong (admin auth was never broken — `requireAdmin` correctly trusts `user_profiles.role` via the service-role client) but its outcome right, for a different reason: `user_profiles` RLS restricted which row an authenticated user could write, never which columns, so a user could PATCH `role`/`pro_status`/`stripe_customer_id` directly on their own row via PostgREST. Proven exploitable with a disposable test account before the fix, proven closed after. Fixed by `trg_user_profiles_column_lockdown` (see the `user_profiles` columns entry above). `.git` was also confirmed live-exposed on `orabo.app` (unrelated to the RLS finding) and closed via `.assetsignore`. A forensic sweep of all `role='admin'`/anomalous-`pro_status` rows found two: the reporter's own throwaway account (reset to free-tier defaults) and one confirmed-legitimate internal admin/test account (left untouched). `SECURITY DEFINER` functions and Edge Functions were checked as a second bypass class — clean (only `purge_account`, already `service_role`-only). `js/dashboard-inline.js`'s `postMessage` listener also got an origin check as a drive-by hardening fix.
### CLOSED 2026-06-09 — Pass 7 RLS Hardening (backend-only tables)
- Closed the Pass 6 F6.5.1 deferral: `analytics_events`, `one_off_purchases`, `ai_cache`, `processed_webhook_events` (OPS) and `post_earn_rewards`, `visits`, `scrape_logs` (Content) all confirmed `relrowsecurity = true`, zero access-granting policies for `anon`/`authenticated`. Only `analytics_events` was genuinely new (added post-Pass-2 via `routes/track.js`); the other six were already covered by the Pass 2 migration and re-verified idempotently. Audit doc: `audit/Orabo_Pass_7_Audit.docx` (backend repo). The backend security-audit series (Passes 1–7) is closed; `user_profiles`/`purchases` (row-scoped) and anon-readable content tables were out of scope here, covered by Pass 2/3.
...
### Low / Parked
- **Hero recenter CLS — `#heroTestimonial` deferred reveal shifts right column (homepage only):** `.hero { align-items: center }` + the `#heroTestimonial` reveal (deferred via `requestIdleCallback`) grows the left flex column ~1.5s after load, recentering the row and producing a ~0.0143 layout shift attributed to whatever sits in the right column (currently `video.hero-demo`; previously the phone mockup — identical behaviour). Total page CLS 0.0146 is already inside Google "good" (≤0.1), so impact is metric-hygiene, not UX-breaking. Root cause is positional recenter, NOT intrinsic resize of the right-column element (video box proven stable at 260×563 from first sample). Candidate fixes (pick at implementation time): reserve the testimonial's vertical space up-front (min-height on its container) so the reveal fills reserved space; OR `.hero { align-items: flex-start }` so left-column growth does not recenter the right column; OR reveal the testimonial via opacity-only with space pre-reserved. Constraint: must not reintroduce CLS elsewhere or break the 1024px mobile hide or the Phase 2 spine/underline. Single commit, Chrome MCP smoke after. Defer — metric hygiene only; behind the NIW content gate.
- **News Phase 2 data-map staleness:** `news_bookmarks` + `news_reactions` are LIVE in OPS (`rrkkbnoqubfmhrnxgngp`, composite PK, owner-only RLS) and exercised by `routes/news-interactions.js` (incl. `DELETE /api/news/:id/react` for reaction toggle-off, symmetric with `DELETE /:id/bookmark`). The backend `audit/privacy-data-map-v1.2.md` still marks them "NOT YET BUILT — Phase 2" — update that marker (and the `to_regclass` relation-missing probe note in `routes/me.js`, now satisfiable) in the post-Phase-2 hygiene pass.
- **Orphaned timeline dead-code cleanup (follow-up to Migration Timeline persistence):** delete `js/dashboard.js` `saveTimelineMilestones()` (and the unused `dashboard.js` if fully dead) and drop the empty `timeline_milestones` OPS table — AFTER the `migration_timeline_state` persistence soaks. Before deleting JS: grep BOTH repos for `saveTimelineMilestones` + any `dashboard.js` import to confirm zero live referents (the GDPR export walks the *table*, not the JS function). Dropping `timeline_milestones` also requires removing its walk from `routes/me.js` and its RLS policies. Surface the grep result before deleting.
...
- **Privacy policy + consent (from Pass 6 F6.10.1/2/4) — CLOSED:** F6.10.1 (privacy.html rewrite, adds Anthropic) shipped (`8d239bc`); F6.10.2 (geo-conditioned cookie consent banner) shipped (`e68e979`, see Coding Rules → Cookie consent). `privacy.html` now declares **PostHog** as a processor (analytics, EU instance `eu.posthog.com`, IP collection disabled, DNT respected, no marketing/advertising use) in section 5, and the cookie clause no longer claims "no tracking cookies".
- **Terms of Service (commit 53542a3):** canonical contract at /terms, ~2,427 words, 13 sections, mirrors privacy policy voice (universal, plain English, no em dashes, British English). Conspicuousness via .policy-caps class with literal uppercase in §2/§4/§10. Counsel-review note in footer.
- **Data-subject-rights endpoints (from Pass 6 F6.10.3) — CLOSED (`8ae7f3b`):** F6.10.4 export-my-data shipped (`GET /api/me/export` + dashboard "Privacy and your data" panel — see Coding Rules → Data export). F6.10.3 delete-account shipped (`8ae7f3b`): delete/restore/deletion-status endpoints, the `hardPurgeAccounts` 04:00 UTC dual-DB purge cron, the `pii_redacted_at` schema addition, and the dashboard delete/restore control, all per `audit/privacy-data-map-v1.2.md @ 08ae25e` §7 purge order. The `purge_account()` RPC already `to_regclass`-guards `news_bookmarks`/`news_reactions` ahead of Phase 2.
