...
## Current State — Phase 8 + Phase 5A/5B + Phase EB Tier 1 + Phase EB Tier 2 + Phase EB Tier 3 + Phase O-1A + Phase UKGT Tier 1 + Email Pipeline Hardening + Review Screen Polish + Back Buttons & Cancel Return + Payment Integrity (Live)

> **Email pipeline last reviewed: 2026-05-13.** Active `[brevo-<context>]` contexts: `checklist`, `report`, `cv`, `cv-attr`, `sop`, `sop-attr`, `eb1a`, `eb1a-attr`, `eb2`, `eb2-attr`, `eb1a-letter`, `eb1a-letter-attr`, `eb2-letter`, `eb2-letter-attr`, `eb1a-package`, `eb1a-package-attr`, `eb2-package`, `eb2-package-attr`, `o1a`, `o1a-attr`, `o1a-letter`, `o1a-letter-attr`, `o1a-package`, `o1a-package-attr`, `ukgt-tech`, `ukgt-tech-attr`, `consultation`, `waitlist-welcome`, `rejection`, `story-approved`, `story-rejected`, `payout`, `opp-ack`, `publish`.
...
- **UKGT Tech Nation route (`routes/ukgt-tech.js`):** mounted at `/api/ukgt-tech`. Single outputType: `ukgt_tech_assessment` → plain text/markdown, Tech Nation eligibility assessment. `sanitizeBrand()` defined inline. UK English mandatory throughout the system prompt — organisation/programme/behaviour/recognise/analyse/specialise etc. (see UKGT_TECH_SYSTEM_PROMPT banned-phrases list). `max_tokens: 8000`. `res.setTimeout(180000)` + `req.setTimeout(180000)`. NO ai_cache reads. verifyPayment + gate before Claude call. One system block with `cache_control: { type: 'ephemeral' }`. `formAnswers` field name in POST body. Filters `ukgt_tech_q0_user_name` and `ukgt_tech_q0_user_email` from user content. Brevo delivery + attribute update fire-and-forget. `[brevo-ukgt-tech]` / `[brevo-ukgt-tech-attr]` log prefixes. `one_off_purchases` audit row written. File key: `ukgt_tech_assessment_docx`. Download filename: `ukgt-tech-nation-eligibility-assessment.docx`.
- **UKGT Tech Nation frontend tool (`ukgt-tech-tool.html`, `js/ukgt-tech-tool.js`):** identical pattern to O-1A Tier 1. Polls `window.ukgtTechTool` (100 ms × 50 attempts). MODAL_ID `ukgt-tech-tool-modal`. sessionStorage keys: `orabo_ukgt_tech_answers` / `orabo_ukgt_tech_output_type`. ITEM_TYPE `ukgt_tech_assessment`. Question ID prefix: `ukgt_tech_q<n>_<slug>`. Email validation checks `q.id === 'ukgt_tech_q0_user_email'`. Section 0 back button → `/uk-global-talent.html` (not `/`). SCREENS constant includes `ukgt-tech-review-screen`. Upsell in download screen links to `/uk-global-talent.html` (Tier 2 waitlist). Mandatory email validation at section 0. No caching, no userId, DOCX-only download. Cancel return restores answers from sessionStorage and shows review screen.
- **css/ibtool.css:** EB + O-1A + UKGT tool modal/screen visibility (`.active` pattern) for Tier 1, Tier 2, and Tier 3 ID sets, including review screen IDs (`eb1a-review-screen`, `eb2-review-screen`, `eb1a-letter-review-screen`, `eb2-letter-review-screen`, `eb1a-package-review-screen`, `eb2-package-review-screen`, `o1a-review-screen`, `o1a-letter-review-screen`, `o1a-package-review-screen`, `ukgt-tech-review-screen`). `.tool-spinner` + `.tool-loading-inner` + `.tool-loading-msg` for loading screen. `.ib-hint-icon` + `.ib-hint-tooltip` tooltip. `.ib-payment-card` + `.ib-payment-features` + `.ib-payment-price` + `.ib-disclaimer` for payment card. When adding a new EB, O-1A, or UKGT tool page, extend the modal/screen ID lists in ibtool.css. Imported in `styles.css` before `mobile.css`.
- **Review screen pattern (all 12 tool modules):** A `*-review-screen` div sits between the last form section and the payment screen in each tool's HTML (`cv-review-screen`, `sop-review-screen`, `eb1a-review-screen`, `eb2-review-screen`, `eb1a-letter-review-screen`, `eb2-letter-review-screen`, `eb1a-package-review-screen`, `eb2-package-review-screen`, `o1a-review-screen`, `o1a-letter-review-screen`, `o1a-package-review-screen`, `ukgt-tech-review-screen`). Rendered by `renderReview()` from `js/tool-review.js` via a thin wrapper in each module. Last form section Next button label: "Review your answers →" (not "Continue to payment →"). Payment screen back button targets the review screen (not the last form section). Review screen back button returns to last form section; continue button advances to payment screen.
...
- **Stripe cancel URL for tool items:** `routes/stripe.js` contains a `TOOL_CANCEL_PAGES` map for the 22 tool item keys (16 CV/SOP/EB/O-1A/UKGT + 6 free tools + consultation). Cancel URL format: `${FRONTEND_URL}/${toolPage}?payment_cancelled=1&item=${itemType}`. Do NOT use `/?payment=cancelled` for these items — that URL has no sessionStorage and loses the user's form answers. Dashboard items still cancel to `dashboard.html?payment=cancelled`; all others to `/?payment=cancelled`.
- **payment_cancelled=1 bypass in stripe.js:** the `checkPaymentReturn` IIFE in `js/stripe.js` preserves the URL (does nothing) when `payment_cancelled=1` is present and the item is one of the 22 tool keys. This allows the tool module's `init()` to read and handle the parameter. Do not clear the URL or show a generic banner for these items.
