...
- **Privacy policy + consent (from Pass 6 F6.10.1/2/4) — PARTIAL:** F6.10.1 (privacy.html rewrite, adds Anthropic) shipped (`8d239bc`); F6.10.2 (geo-conditioned cookie consent banner) shipped (`e68e979`, see Coding Rules → Cookie consent). **Remaining:** `privacy.html` still omits **PostHog** as a processor (loads sitewide via `js/analytics.js`, EU instance, no IP, DNT-respected) and the cookie clause still claims "no tracking cookies" — correct the cookie clause and add PostHog as a processor. Prince approves copy.
...
- **Data-subject-rights endpoints (from Pass 6 F6.10.3) — PARTIAL:** F6.10.4 export-my-data shipped (`GET /api/me/export` + dashboard "Privacy and your data" panel — see Coding Rules → Data export). **Remaining (F6.10.3):** no delete-account endpoint/UI yet — `privacy.html` promises 30-day account-data deletion but deletion is still manual email only. Build the delete cron/endpoint per `audit/privacy-data-map-v1.2.md @ 08ae25e` §7 purge order (`pii_redacted_at` schema addition is part of that build) + a dashboard control alongside the existing export button.
