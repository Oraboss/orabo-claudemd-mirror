...
- **Digest `sent` count and the `digest_sends` ledger key on Brevo *acceptance*, not delivery.** `sendTransactionalEmail` (`utils/brevo-email.js`) treats any axios-resolved `2xx` from `/smtp/email` as success and logs the returned messageId (`[brevo-send] accepted messageId=… to=…`); a `2xx` + messageId means Brevo QUEUED the message, not that it reached an inbox. Blocks/bounces (e.g. prior-hardbounce suppression — a Brevo-internal transactional suppression list, distinct from the contact-level `emailBlacklisted` flag the loop checks) are resolved asynchronously and never surface in the synchronous response, so `digest.js` increments `sent` and writes the `digest_sends` row on acceptance. Consequence: a transient block/bounce still gets a ledger row, and a non-`force` retry then `skipped_already`-skips it — legitimate retries can be silently suppressed. The proper fix is a Brevo delivery/bounce webhook that reconciles the ledger against actual delivery status (**scoped follow-up — not built**). Until then, confirm real delivery via Brevo `/smtp/statistics/events` (or the dashboard), never from the `sent` count alone.
