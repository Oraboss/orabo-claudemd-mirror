...
- **userId in AI generation requests:** `payment-return-checklist.js` / `payment-return-quiz.js` pass `userId: state.userId || null` directly from saved state — no `supabaseClient.auth.getSession()` call before fetch (that was blocking the API call). Pro-bypass scripts still call `getSession()` for their own flow. Do NOT pass `userId` to `/api/cv` or `/api/sop` — these are anonymous; use `formAnswers.user_email` instead
...
- **CV/SOP purchase verification:** if `userId` is present in request body, routes check `purchases` table (logged-in users on dashboard.html). If absent (anonymous cv-tool.html / sop-tool.html flow), the purchases check is skipped — payment is trusted via Stripe redirect. Do not add a hard `userId` requirement to `/api/cv` or `/api/sop`.
...
- **Pro bypass scripts must NOT call `supabaseClient.auth.getSession()` before the API call** — it hangs indefinitely in the iframe context. The getSession call at the bottom of DOMContentLoaded is for Pro status detection only (slow path), not for the API fetch. Pass `userId: null` directly in the API body; backend handles anonymous Pro flows.
- **Email delivery for checklist + report uses `user_email` from request body**, not userId lookup. Frontend reads from `localStorage.orabo_user_email` (set by dashboard.html on auth, cleared on sign-out). Backend in `routes/checklist.js` and `routes/report.js` falls back to userId Supabase lookup if `user_email` is absent. Resolution priority: `user_email` → userId lookup → skip send + log `[brevo-checklist] no recipient email available`. Both routes return `email_queued: boolean` in the response; frontend email-notice is gated on `json.email_queued`.
