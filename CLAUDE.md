...
- **Sitewide nav HTML is duplicated across 9 pages**, not templated: `index.html`, `pathway-compare.html`, `take-home-pay.html`, `cost-of-living.html`, `processing-times.html`, `universities.html`, `scholarships.html`, `visa-pathways.html`, `opportunities.html`. Any change to the Tools dropdown, Explore dropdown, or top-level nav links MUST be applied to all 9 files in the same commit. Pre-commit verification: `for f in index.html pathway-compare.html take-home-pay.html cost-of-living.html processing-times.html universities.html scholarships.html visa-pathways.html opportunities.html; do grep -c "nav-group-label" "$f"; done` — all must return identical counts. Until a template/include system is introduced, this is the only safeguard. Note: hub pages use `/#readiness`, `/#eligibility`, `/#doc-checklist` anchors (root-relative back to index.html); `index.html` uses bare `#` anchors — never copy verbatim without adjusting.
- **Architecture follow-up (not blocking):** the nav duplication above is a maintenance liability. Future work should extract the nav into a shared partial loaded by a build step or a runtime fetch-and-inject pattern. Defer until at least 3 nav-related changes have shipped — premature abstraction is more expensive than disciplined duplication for a 9-file scope.
