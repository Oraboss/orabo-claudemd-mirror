...
- `budget_tracker`: Empty user-id-keyed OPS table. Defensively swept by F6.10.3 delete cron and enumerated as [] by F6.10.4 export. Origin unverified; if a future feature claims this table name, audit before reuse.
- `timeline_milestones`: Empty user-id-keyed OPS table. Defensively swept by F6.10.3 delete cron and enumerated as [] by F6.10.4 export. Origin unverified; if a future feature claims this table name, audit before reuse. NOT the live Migration Timeline store — that is `migration_timeline_state` (below). The orphaned `js/dashboard.js saveTimelineMilestones()` wrote this table via the old browser-direct RLS path; it is dead (imported by no HTML) and slated for removal.
